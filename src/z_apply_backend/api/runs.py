...
async def start_run(
    ...
    return RunResponse.model_validate(await handle.view(), from_attributes=True, by_name=False)
...
async def get_run_detail(request: __import__("fastapi").Request, run_id: UUID) -> RunResponse:
    async with request.app.state.sessions() as session:
...
async def cancel_run(run_id: UUID, app_core: ZApplyCore = Depends(core)) -> RunResponse:
    ...
    return RunResponse.model_validate(await handle.view(), from_attributes=True)
...
async def focus_run(run_id: UUID, app_core: ZApplyCore = Depends(core)) -> RunResponse:
    ...
    if handle is None:
        raise HTTPException(404, detail={"code": "run_not_found"})
    try:
        return RunResponse.model_validate(await handle.focus_browser(), from_attributes=True)
...
async def close_browser(run_id: UUID, app_core: ZApplyCore = Depends(core)) -> RunResponse:
    ...
    if handle is None:
        raise HTTPException(404, detail={"code": "run_not_found"})
    try:
        return RunResponse.model_validate(await handle.close_browser(), from_attributes=True)
    except Exception as exc:
        raise _integration_error(exc) from None
...
async def run_events(
    request: __import__("fastapi").Request, run_id: UUID, after: int = 0
